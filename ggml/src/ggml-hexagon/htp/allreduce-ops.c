...
static void allreduce_thread_dma_1d_##SUFFIX(unsigned int nth, unsigned int ith, void * data) {             \
    ...
    for (uint32_t ir = ir0; ir < ir1; ) {                                                                   \
        ...
        for (uint32_t d = 0; d < n_dsts; d++) {                                                             \
            ...
            dma_queue_push(q, dma_make_data(d_ddr, d_spad), cur_bytes, cur_bytes, cur_bytes, 1);            \
        }                                                                                                   \
        if (ir_prefetch < ir1) {                                                                            \
            ...
            for (uint32_t s = 0; s < n_ranks; s++) {                                                        \
                ...
                dma_queue_push(q, dma_make_data(s_spad[s], s_next), next_bytes, next_bytes, next_bytes, 1); \
            }                                                                                               \
            if (HAS_ADD) {                                                                                  \
                const dma_addr_t r_next = octx->src[2 * n_ranks]->data + ir_prefetch * sizeof(TYPE);        \
                dma_queue_push(q, dma_make_data(r_spad, r_next), next_bytes, next_bytes, next_bytes, 1);    \
            }                                                                                               \
...
static void allreduce_thread_dma_2d_##SUFFIX(unsigned int nth, unsigned int ith, void * data) {                                                   \
    ...
    for (int k = 0; k < 2 && r_prefetch < r1; k++) {                                                                                              \
        ...
        for (uint32_t d = 0; d < n_dsts; d++) {                                                                                                   \
            ...
            dma_queue_push(q, dma_make_data(d_ddr, d_spad), octx->dsts[d]->nb[1], row_size_aligned, row_bytes, 0);                                \
        }                                                                                                                                         \
        for (uint32_t s = 0; s < n_ranks; s++) {                                                                                                  \
            uint8_t * s_spad = src_spad_base[s] + spad_idx * spad_half;                                                                           \
            const dma_addr_t s_ddr = octx->src[s]->data + r_prefetch * octx->src[s]->nb[1];                                                       \
            dma_queue_push(q, dma_make_data(s_spad, s_ddr), row_size_aligned, octx->src[s]->nb[1], row_bytes, cur_rows);                          \
        }                                                                                                                                         \
        if (HAS_ADD && !IS_ROW_BCAST) {                                                                                                           \
            uint8_t * r_spad = res_spad_base + spad_idx * spad_half;                                                                              \
            const dma_addr_t r_ddr = octx->src[2 * n_ranks]->data + r_prefetch * octx->src[2 * n_ranks]->nb[1];                                   \
            dma_queue_push(q, dma_make_data(r_spad, r_ddr), row_size_aligned, octx->src[2 * n_ranks]->nb[1], row_bytes, cur_rows);                \
        }                                                                                                                                         \
        r_prefetch += cur_rows;                                                                                                                   \
        spad_idx ^= 1;                                                                                                                            \
    }                                                                                                                                             \
    ...
    for (uint32_t r = r0; r < r1; ) {                                                                                                             \
        ...
        for (uint32_t d = 0; d < n_dsts; d++) {                                                                                                   \
            ...
            dma_queue_push(q, dma_make_data(d_ddr, d_spad), octx->dsts[d]->nb[1], row_size_aligned, row_bytes, cur_rows);                         \
        }                                                                                                                                         \
        if (r_prefetch < r1) {                                                                                                                    \
            uint32_t next_rows = MIN(block_rows, r1 - r_prefetch);                                                                                \
            for (uint32_t s = 0; s < n_ranks; s++) {                                                                                              \
                const dma_addr_t s_next = octx->src[s]->data + r_prefetch * octx->src[s]->nb[1];                                                  \
                dma_queue_push(q, dma_make_data(s_spad[s], s_next), row_size_aligned, octx->src[s]->nb[1], row_bytes, next_rows);                 \
            }                                                                                                                                     \
            if (HAS_ADD && !IS_ROW_BCAST) {                                                                                                       \
                const dma_addr_t r_next = octx->src[2 * n_ranks]->data + r_prefetch * octx->src[2 * n_ranks]->nb[1];                              \
                dma_queue_push(q, dma_make_data(r_spad, r_next), row_size_aligned, octx->src[2 * n_ranks]->nb[1], row_bytes, next_rows);          \
            }                                                                                                                                     \
...
int op_allreduce(struct htp_ops_context * octx) {
    ...
    if (nelem > 0) {
        ...
        if (has_add && actx.is_row_bcast) {
            ...
            dma_queue_push(q, dma_make_data(actx.res_spad_base, r_ddr), actx.row_size_aligned, 0, row_bytes, 1);
            dma_queue_pop(q);
        }
