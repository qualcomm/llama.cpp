...
#define DEFINE_GLU_PER_THREAD_IMPL(NAME, TYPE, SUFFIX, OP_STR, CORE_EXPR)                                              \
    static void glu_##NAME##_##SUFFIX##_per_thread(unsigned int nth, unsigned int ith, void * data) {                  \
        ...
        for (uint32_t ir = src0_start_row; ir < src0_end_row; ir += BLOCK) {                                           \
            const uint32_t block_size = MIN(BLOCK, src0_end_row - ir);                                                 \
                                                                                                                       \
            TYPE * dst_spad  = (TYPE *) dma_queue_pop(dma_queue).src;                                                  \
            TYPE * src0_spad = (TYPE *) dma_queue_pop(dma_queue).dst;                                                  \
            TYPE * src1_spad = (TYPE *) dma_queue_pop(dma_queue).dst;                                                  \
                                                                                                                       \
            htp_trace_event_start(tr, HTP_TRACE_EVT_HVX_COMP, ir);                                                     \
            CORE_EXPR;                                                                                                 \
            htp_trace_event_stop(tr, HTP_TRACE_EVT_HVX_COMP, ir);                                                      \
                                                                                                                       \
            dma_queue_push(dma_queue, dma_make_ptr(data_dst + (ir * dst_row_size), dst_spad),                          \
...
// F32 GLU task: row-block DMA/VTCM plumbing, float-typed VTCM buffers.
#define DEFINE_GLU_PER_THREAD(NAME, OP_STR, CORE_EXPR) \
    DEFINE_GLU_PER_THREAD_IMPL(NAME, float, f32, OP_STR, CORE_EXPR)
...
// F16 GLU tasks: same DMA/VTCM plumbing as DEFINE_GLU_PER_THREAD, but VTCM buffers are
// _Float16-typed.
DEFINE_GLU_PER_THREAD_IMPL(swiglu, _Float16, f16, "swiglu-f16", swiglu_f16(src0_spad, src1_spad, dst_spad, block_size, actx))
DEFINE_GLU_PER_THREAD_IMPL(swiglu_oai, _Float16, f16, "swiglu-oai-f16", swiglu_oai_f16(src0_spad, src1_spad, dst_spad, block_size, actx))
DEFINE_GLU_PER_THREAD_IMPL(swiglu_clamp, _Float16, f16, "swiglu-clamp-f16", swiglu_clamp_f16(src0_spad, src1_spad, dst_spad, block_size, actx))
DEFINE_GLU_PER_THREAD_IMPL(geglu, _Float16, f16, "geglu-f16", geglu_f16(src0_spad, src1_spad, dst_spad, block_size, actx))
...
static int execute_op_activations_f32(struct htp_ops_context * octx) {
    ...
    work_queue_func_t act_op_func;
    const char *      op_type = NULL;

    switch (octx->op) {
        case HTP_OP_GLU_SWIGLU:
            act_op_func = (work_queue_func_t)glu_swiglu_f32_per_thread;
            op_type     = "swiglu-f32";
            break;

        case HTP_OP_GLU_SWIGLU_OAI:
            act_op_func = (work_queue_func_t)glu_swiglu_oai_f32_per_thread;
            op_type     = "swiglu-oai-f32";
            break;

        case HTP_OP_GLU_SWIGLU_CLAMP:
            act_op_func = (work_queue_func_t) glu_swiglu_clamp_f32_per_thread;
            op_type     = "swiglu-clamp-f32";
            break;

        case HTP_OP_GLU_GEGLU:
            act_op_func = (work_queue_func_t)glu_geglu_f32_per_thread;
            op_type     = "geglu-f32";
            break;

        case HTP_OP_GLU_GEGLU_QUICK:
            act_op_func = (work_queue_func_t)glu_geglu_quick_f32_per_thread;
...
static int execute_op_activations_f16(struct htp_ops_context * octx) {
    ...
    switch (octx->op) {
        case HTP_OP_GLU_SWIGLU:
            act_op_func = (work_queue_func_t)glu_swiglu_f16_per_thread;
            op_type     = "swiglu-f16";
            break;

        case HTP_OP_GLU_SWIGLU_OAI:
            act_op_func = (work_queue_func_t)glu_swiglu_oai_f16_per_thread;
            op_type     = "swiglu-oai-f16";
            break;

        case HTP_OP_GLU_SWIGLU_CLAMP:
            act_op_func = (work_queue_func_t) glu_swiglu_clamp_f16_per_thread;
            op_type     = "swiglu-clamp-f16";
            break;

        case HTP_OP_GLU_GEGLU:
            act_op_func = (work_queue_func_t)glu_geglu_f16_per_thread;
            op_type     = "geglu-f16";
            break;
        default:
            FARF(ERROR, "Unsupported activations Op %u\n", octx->op);
    ...
    const size_t nc_bytes        = dst->ne[0] * SIZEOF_FP16;
    const size_t src0_row_size   = nc_bytes;
    const size_t src1_row_size   = nc_bytes;
