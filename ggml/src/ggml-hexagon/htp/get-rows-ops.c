...
static void get_rows_thread_##TYPE_NAME##_##IDX_TYPE(unsigned int nth, unsigned int ith, void *data) {                 \
    ...
    for (uint32_t step = 0; step < ir1 - ir0; ++step) {                                                                \
        ...
        if (next_step < ir1 - ir0) {                                                                                   \
            ...
            const uintptr_t psrc0_ptr = octx->src[0]->data + pi01*nb01 + pi02*nb02 + pi03*nb03;                        \
            dma_queue_push(dma_queue,                                                                                  \
...
GET_ROWS_THREAD_DT_FN(f32, int32_t, {
    ...
    if (offset < ne00) {
        ...
        hvx_copy_f32_uu((uint8_t *)dst_spad + offset * sizeof(float), (const uint8_t *)src_spad + offset * sizeof(float), copy_size);
    }
})
