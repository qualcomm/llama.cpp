...
struct htp_pad_context {
    ...
    int32_t  lp0, rp0;   // dim 0 pad amounts
    int32_t  lp1, rp1;   // dim 1 pad amounts
    int32_t  lp2, rp2;   // dim 2 pad amounts
    int32_t  lp3, rp3;   // dim 3 pad amounts
    ...
    size_t   src_row_size;          // ne00 * type_size
    size_t   src_row_size_aligned;  // hex_round_up(src_row_size, VLEN)
    size_t   dst_row_size;          // ne0  * type_size
    size_t   dst_row_size_aligned;  // hex_round_up(dst_row_size, VLEN)
};

// Common preamble for all PAD kernel variants: tensor pointers, ne/nb, pad params,
// type_size, and row range.  Unused variables are suppressed by the -Wunused-variable
// pragma at the top of this file (same pattern as htp_unary_preamble / htp_binary_preamble).
...
// Additional preamble for DMA kernel variants: row sizes and VTCM spad pointers.
// Must be used after htp_pad_preamble (requires pctx and octx to be in scope).
...
static void pad_job_per_thread_hvx(unsigned int nth, unsigned int ith, void * data) {
    ...
    for (uint32_t dst_row = row_start; dst_row < row_end; dst_row++) {
        ...
        // Check if this row is in the interior (has src data) or in the padded border.
        ...
        if (!interior) {
            // Entire row is in the padded border — HVX zero-fill the whole row
            hvx_splat_f32_u(dst_ptr, 0.0f, ne0);
        } else {
            // Row is in the interior — map to src row via offset
            const uint8_t * src_ptr = pad_src_row_ptr(src, i1, i2, i3, lp1, lp2, lp3);

            // Left pad in dim 0: HVX zero-fill
            if (lp0 > 0) {
            ...
            // Right pad in dim 0: HVX zero-fill
            if (rp0 > 0) {
...
static void pad_job_per_thread_hvx_dma(unsigned int nth, unsigned int ith, void * data) {
    ...
    //
    // The dummy dst DMA (nrows=0) establishes queue ordering and completes
    // immediately.  Its .src field carries the VTCM buffer pointer back to
    // the main loop via dma_queue_pop().
    // -----------------------------------------------------------------------
    for (uint32_t ir = row_start, spad_idx = 0; ir < row_end && spad_idx < 2; ir++, spad_idx++) {
        ...
        // Dummy dst DMA (nrows=0): completes immediately, carries dst_spad_cur
        dma_queue_push_vtcm_to_ddr(dma,
            dma_make_ptr((uint8_t *)dst->data, dst_spad_cur),
            dst_row_size, dst_row_size_aligned, 0);

        // Determine whether this row is interior or a border row
        uint32_t i1, i2, i3;
        ...
        // Border row: null DMA (nrows=0) — completes immediately, hardware
    ...
    for (uint32_t ir = row_start; ir < row_end; ir++) {
        // Recover VTCM buffer pointers from the DMA queue.
        // dst_spad_cur comes from the dummy VTCM→DDR DMA (.src = VTCM ptr).
        // src_spad_cur comes from the DDR→VTCM DMA (.dst = VTCM ptr).
        ...
        if (!interior) {
            // Border row: zero-fill the entire dst row in VTCM (aligned).
            hvx_splat_f32_a(dst_spad_cur, 0.0f, ne0);
        } else {
            // Interior row: zero-fill the entire dst row first (covers left
            hvx_splat_f32_a(dst_spad_cur, 0.0f, ne0);
        ...
        // Push dst DMA: write the assembled row from VTCM back to DDR.
        dma_queue_push_vtcm_to_ddr(dma,
            dma_make_ptr(dst_ptr, dst_spad_cur),
            dst_row_size, dst_row_size_aligned, 1);

        // Prefetch src for row ir+2 (double-buffering: while we process row
...
static void pad_job_per_thread_hvx_circular(unsigned int nth, unsigned int ith, void * data) {
    ...
    for (uint32_t dst_row = row_start; dst_row < row_end; dst_row++) {
        ...
        if (nb00 == type_size) {
            ...
            // Middle: full source row → dst[lp0 .. lp0 + ne00)
            hvx_copy_f32_uu(dst_ptr + (size_t)lp0 * type_size, src_row, ne00);
            // Right pad: src[0 .. rp0) → dst[lp0 + ne00 .. ne0)
            if (rp0 > 0) {
                if ((uint32_t)rp0 < 32) {
            ...
            // Non-contiguous dim 0 (permuted tensor): element-by-element copy
            // Left pad: src[ne00 - lp0 .. ne00) → dst[0 .. lp0)
            for (uint32_t i = 0; i < (uint32_t)lp0; i++) {
                *(float *)(dst_ptr + i * type_size) =
                    *(const float *)(src_row + (size_t)(ne00 - (uint32_t)lp0 + i) * nb00);
            }
            // Middle: src[0 .. ne00) → dst[lp0 .. lp0 + ne00)
            for (uint32_t i = 0; i < ne00; i++) {
...
static void pad_job_per_thread_hvx_circular_dma(unsigned int nth, unsigned int ith, void * data) {
    ...
    for (uint32_t ir = row_start; ir < row_end; ir++) {
        ...
        // Assemble the circular output row in VTCM.
        //
        // Left wrap: src_spad[ne00-lp0 .. ne00) → dst_spad[0 .. lp0)
        if (lp0 > 0) {
            ...
            if ((uint32_t)lp0 < 32) {
                ...
                // dst_left is VTCM-aligned; src_left alignment depends on ne00-lp0.
                hvx_copy_f32_uu(dst_left, src_left, (uint32_t)lp0);
            }
        }

        // Middle: src_spad[0 .. ne00) → dst_spad[lp0 .. lp0+ne00)
        {
            uint8_t * dst_mid = dst_spad_cur + (size_t)lp0 * type_size;
            // src_spad_cur is VTCM-aligned; dst_mid is aligned iff lp0 % 32 == 0.
        ...
        // Prefetch src for row ir+2 (double-buffering).
        const uint32_t next_row = ir + 2;
        if (next_row < row_end) {
