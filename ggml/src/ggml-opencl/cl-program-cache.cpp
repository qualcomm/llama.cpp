...
static std::string compute_key(const std::string & key_suffix,
                               const char *        source,
...
static bool make_dir_recursive(const std::string & path) {
    ...
    if (slash != std::string::npos && slash > 0) {
        ...
        if (!make_dir_recursive(parent)) return false;
    }
#if defined(_WIN32)
...
static std::string default_cache_dir() {
    ...
    if (xdg && *xdg) return std::string(xdg) + "/llama.cpp/cl-cache";
    const char * home = std::getenv("HOME");
    if (!home || !*home) home = ".";
    return std::string(home) + "/.cache/llama.cpp/cl-cache";
#endif
}

// Query a NUL-terminated string from clGetDeviceInfo / clGetPlatformInfo.
template <typename GetInfoFn, typename Object>
static std::string query_string(GetInfoFn fn, Object obj, cl_uint name) {
    size_t sz = 0;
    if (fn(obj, name, 0, nullptr, &sz) != CL_SUCCESS || sz == 0) return {};
    std::string s(sz, '\0');
    if (fn(obj, name, sz, &s[0], nullptr) != CL_SUCCESS) return {};
    if (!s.empty() && s.back() == '\0') s.pop_back();
    return s;
}

static std::string compute_key_suffix(cl_device_id device) {
...
static const uint8_t MAGIC[8] = { 'G','G','M','L','C','L','B','C' };
...
static bool write_atomic(const std::string & path, const uint8_t * data, size_t len) {
    std::string tmp = path + ".tmp." + std::to_string(ggml_getpid());
    {
...
#if defined(_WIN32)
    // MoveFileEx with REPLACE_EXISTING is atomic on the same volume for NTFS.
...
// One-time read of GGML_OPENCL_KERNEL_CACHE_DEBUG. When set (any non-empty
// value), try_load / try_save log a one-line trace per call: HIT / MISS /
// SAVE, the SHA-256 key, the source size, and the first ~120 chars of the
// compile_opts string. Used to diagnose unexpected cache misses (e.g. when
// two dispatches we expected to share a key produce different keys because
// some -D flag is parameterised by runtime state).
...
void cl_program_cache_try_save(
    ...
    if (state.dir.empty() || !program || !source) return;

    cl_uint n_dev = 0;
    if (clGetProgramInfo(program, CL_PROGRAM_NUM_DEVICES, sizeof(n_dev), &n_dev, nullptr) != CL_SUCCESS || n_dev == 0) return;

    std::vector<size_t> sizes(n_dev);
    if (clGetProgramInfo(program, CL_PROGRAM_BINARY_SIZES, sizeof(size_t) * n_dev, sizes.data(), nullptr) != CL_SUCCESS) return;
    if (sizes.empty() || sizes[0] == 0) return;
