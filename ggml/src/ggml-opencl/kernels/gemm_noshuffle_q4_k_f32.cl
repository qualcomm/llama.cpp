...
// lane consumed only HALF a uint32 texel, so the sampler moved twice the texels for the
// same bytes. Four rows per lane is one ushort4 = 8 B = two WHOLE texels, so that
// premise no longer applies. Worth retrying because the q4_K GEMV reads its weights
...
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_wimg(
    read_only image1d_buffer_t src0_q_img,
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
    ...
    for (int blk = sg; blk < num_32blk; blk += COK_NSG) {
        ...
        for (int l = 0; l < 32; l += 4) {
            int ki = i + l;
            // 8 bytes per lane: this lane's four rows at this K-group
            // 8 bytes per lane = TWO whole uint32 texels. The 1-row texture variant
            // took half a texel per lane, which is why it lost 39%; at 4 rows that
            // premise no longer holds, and the GEMV reaches 117 GB/s through exactly
            // this read path where the buffer read here manages 88.
            uint tex = ((uint)row0 + (uint)(ki >> 2) * (uint)m) >> 1;
            uint w0 = read_imageui(src0_q_img, (int)(tex    )).x;
            uint w1 = read_imageui(src0_q_img, (int)(tex + 1)).x;
            ushort4 bits = (ushort4)((ushort)(w0 & 0xFFFFu), (ushort)(w0 >> 16),
                                     (ushort)(w1 & 0xFFFFu), (ushort)(w1 >> 16));
...
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_wimg_nr(
    read_only image1d_buffer_t src0_q_img,
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
    ...
    // Both measured wins in one kernel: weights through the texture, and the four
    // outputs in NAMED REGISTERS rather than the private float8 out[4] that costs
    // 128 B per work-item.
    local float8 reduceLM[COK_SG * (COK_NSG - 1)];
    float8 o0 = 0, o1 = 0, o2 = 0, o3 = 0;
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg > 0) { reduceLM[(sg - 1) * COK_SG + lane] = convert_float8(acc2); }
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg == 0) {
        float8 sum = convert_float8(acc2);
        for (int s = 0; s < COK_NSG - 1; s++) { sum += reduceLM[s * COK_SG + lane]; }
        o2 = sum;
    }

    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg > 0) { reduceLM[(sg - 1) * COK_SG + lane] = convert_float8(acc3); }
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg == 0) {
        float8 sum = convert_float8(acc3);
        for (int s = 0; s < COK_NSG - 1; s++) { sum += reduceLM[s * COK_SG + lane]; }
        o3 = sum;
    }
    ...
    if (sg == 0) {
        int idx = row0;
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s0, o1.s0, o2.s0, o3.s0), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s1, o1.s1, o2.s1, o3.s1), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s2, o1.s2, o2.s2, o3.s2), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s3, o1.s3, o2.s3, o3.s3), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s4, o1.s4, o2.s4, o3.s4), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s5, o1.s5, o2.s5, o3.s5), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s6, o1.s6, o2.s6, o3.s6), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)(o0.s7, o1.s7, o2.s7, o3.s7), 0, dst + idx); idx += m; }
    }
}

// _cok_r4_wimg_nr with the ACTIVATION texel widened from 8 B to 16 B. The kernel is
// byte-for-byte the same arithmetic on the same bytes; only the view changes. The
// activation image is bound as CL_RGBA/CL_UNSIGNED_INT32 instead of CL_RGBA/
// CL_HALF_FLOAT, so one read_imageui returns all eight halves a lane needs and the
// inner loop issues ONE image read per K element instead of two.
//
// Two reasons to expect this to pay where the WEIGHT texture did not. First, cok is
// issue-bound rather than bandwidth-bound (that is why 4 rows per lane won), and this
// removes one issue slot per K element without moving an extra byte. Second, it is the
// granularity condition the weight-texture note above states: a lane now consumes a
// WHOLE texel per read instead of half of one.
//
// The transposed activations are still WRITTEN through the half4 view by
...
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_wimg_nr_w8(
    read_only image1d_buffer_t src0_q_img,
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
    ...
    for (int blk = sg; blk < num_32blk; blk += COK_NSG) {
        ...
        for (int l = 0; l < 32; l += 4) {
            ...
            B = as_half8(read_imageui(src1, (ki+0) * n_8));
            acc0 += B * ((half)( bits.s0        & 0x000F) * scale.s0 - mval.s0);
            acc1 += B * ((half)( bits.s1        & 0x000F) * scale.s1 - mval.s1);
            acc2 += B * ((half)( bits.s2        & 0x000F) * scale.s2 - mval.s2);
            acc3 += B * ((half)( bits.s3        & 0x000F) * scale.s3 - mval.s3);

            B = as_half8(read_imageui(src1, (ki+1) * n_8));
            acc0 += B * ((half)((bits.s0 >> 4)  & 0x000F) * scale.s0 - mval.s0);
            acc1 += B * ((half)((bits.s1 >> 4)  & 0x000F) * scale.s1 - mval.s1);
            acc2 += B * ((half)((bits.s2 >> 4)  & 0x000F) * scale.s2 - mval.s2);
            acc3 += B * ((half)((bits.s3 >> 4)  & 0x000F) * scale.s3 - mval.s3);

            B = as_half8(read_imageui(src1, (ki+2) * n_8));
            acc0 += B * ((half)((bits.s0 >> 8)  & 0x000F) * scale.s0 - mval.s0);
            acc1 += B * ((half)((bits.s1 >> 8)  & 0x000F) * scale.s1 - mval.s1);
            acc2 += B * ((half)((bits.s2 >> 8)  & 0x000F) * scale.s2 - mval.s2);
            acc3 += B * ((half)((bits.s3 >> 8)  & 0x000F) * scale.s3 - mval.s3);

            B = as_half8(read_imageui(src1, (ki+3) * n_8));
            acc0 += B * ((half)((bits.s0 >> 12) & 0x000F) * scale.s0 - mval.s0);
            acc1 += B * ((half)((bits.s1 >> 12) & 0x000F) * scale.s1 - mval.s1);
            acc2 += B * ((half)((bits.s2 >> 12) & 0x000F) * scale.s2 - mval.s2);
            acc3 += B * ((half)((bits.s3 >> 12) & 0x000F) * scale.s3 - mval.s3);
...
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_nr(
    global const ushort * src0_q,
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
    ...
    // Cross-subgroup reduction, with the four outputs held as NAMED REGISTERS rather
    // than the private `float8 out[4]` that _cok_r4 uses. On Adreno an indexed
    // per-work-item array lands in private memory: the driver reports
    // CL_KERNEL_PRIVATE_MEM_SIZE 400 B for r4 against 128 B for the 1-row kernel,
    // and the 8-row kernel -- twice the accumulators but all of them named --
    // reports LESS (368 B). This changes the storage class and nothing else.
    local float8 reduceLM[COK_SG * (COK_NSG - 1)];
    float8 o0 = 0, o1 = 0, o2 = 0, o3 = 0;
    ...
    if (sg == 0) {
        int idx = row0;
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s0, (float)o1.s0, (float)o2.s0, (float)o3.s0), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s1, (float)o1.s1, (float)o2.s1, (float)o3.s1), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s2, (float)o1.s2, (float)o2.s2, (float)o3.s2), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s3, (float)o1.s3, (float)o2.s3, (float)o3.s3), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s4, (float)o1.s4, (float)o2.s4, (float)o3.s4), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s5, (float)o1.s5, (float)o2.s5, (float)o3.s5), 0, dst + idx); idx += m; }
        if (idx < m*n_no_padding) { vstore4((float4)((float)o0.s6, (float)o1.s6, (float)o2.s6, (float)o3.s6), 0, dst + idx); idx += m; }
...
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_nrh(
    global const ushort * src0_q,
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
    ...
    // and the 8-row kernel -- twice the accumulators but all of them named --
    // reports LESS (368 B). This changes the storage class and nothing else.
    // AND the reduction buffer is half8, halving LDS from 14336 to 7168 B per
    // workgroup. The K-split partials are summed in half, so this one is NOT
    // bit-identical; it is the LDS-occupancy half of the experiment.
    local half8 reduceLM[COK_SG * (COK_NSG - 1)];
    half8 o0 = 0, o1 = 0, o2 = 0, o3 = 0;

    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg > 0) { reduceLM[(sg - 1) * COK_SG + lane] = acc0; }
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg == 0) {
        half8 sum = acc0;
        for (int s = 0; s < COK_NSG - 1; s++) { sum += reduceLM[s * COK_SG + lane]; }
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg > 0) { reduceLM[(sg - 1) * COK_SG + lane] = acc1; }
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg == 0) {
        half8 sum = acc1;
        for (int s = 0; s < COK_NSG - 1; s++) { sum += reduceLM[s * COK_SG + lane]; }
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg > 0) { reduceLM[(sg - 1) * COK_SG + lane] = acc2; }
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg == 0) {
        half8 sum = acc2;
        for (int s = 0; s < COK_NSG - 1; s++) { sum += reduceLM[s * COK_SG + lane]; }
    ...
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg > 0) { reduceLM[(sg - 1) * COK_SG + lane] = acc3; }
    barrier(CLK_LOCAL_MEM_FENCE);
    if (sg == 0) {
        half8 sum = acc3;
        for (int s = 0; s < COK_NSG - 1; s++) { sum += reduceLM[s * COK_SG + lane]; }
