...
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_wimg_nr_bin_r32(
    ...
    for (int blk = sg; blk < num_32blk; blk += COK_NSG) {
        ...
        for (int l = 0; l < 32; l += 8) {
            int ki = i + l;
            // Four R32UI reads instead of one RGBA32UI texel: slower when the launch fits in
            // one wave of workgroups, faster above it (X2-90, test-backend-ops perf).
            uint tex = (uint)(ki >> 3) * (uint)m + (uint)row0;
            uint4 w = (uint4)(read_imageui(src0_q_img, (int)(tex    )).x,
                              read_imageui(src0_q_img, (int)(tex + 1)).x,
                              read_imageui(src0_q_img, (int)(tex + 2)).x,
                              read_imageui(src0_q_img, (int)(tex + 3)).x);
            ushort4 lo = convert_ushort4(w & 0xFFFFu);
            ushort4 hi = convert_ushort4(w >> 16);
            COK_BIN_K4(lo, ki)
            COK_BIN_K4(hi, ki + 4)
...
// A/B only: the first form of the bin kernel, verbatim.
#ifdef ADRENO_GPU
REQD_SUBGROUP_SIZE_64
#endif
kernel void kernel_gemm_noshuffle_q4_k_f32_cok_r4_wimg_nr_bin_v0(
    read_only image1d_buffer_t src0_q_img,   // bin layout, R32UI, texel u*m + row
    global const uchar  * src0_s,
    global const half   * src0_d,
    global const half   * src0_dm,
