...
// Same 32-K dot, but the token's eight activation uints arrive as two uint4.
// They are contiguous, so staging the LDS tile as uint4 cuts the inner loop's
// __local load count 4x and widens the cooperative global load from 4 to 16
// bytes per lane. The arithmetic is unchanged, so results are byte-identical.
