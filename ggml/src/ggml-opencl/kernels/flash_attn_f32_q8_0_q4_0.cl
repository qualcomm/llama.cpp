...
// Flash attention: Q=f32, K=q8_0, V=q4_0 (asymmetric native KV).
// The K path is identical to flash_attn_f32_q8_0.cl (dp4a QK dot). Only the V
// path differs: V blocks are q4_0 (half d + uchar qs[16]; qs[j] low/high nibble
// -> elem j / j+16, val = d*(nibble-8)). Motivated by gpt-oss, where K carries
// attention-sink outliers that need q8 precision but V tolerates q4 losslessly.
...
// leaves headroom under FLT_MAX so (m_i - score) cannot overflow. It is
// also required for correctness in the N_SPLIT>1 prefill reduction: a
// shuffle-combined whole-tile-masked partial of -INFINITY produces NaN.
...
// V is q4_0: block = 2B scale (half) + 16B nibbles (32 elems). QK4_0==QK8_0==32
// so the per-block count and all DV_VEC / local-array dims match the q8_0 path.
#define QK4_0 32
...
// V dequant (q4_0). out[0..3] = low nibbles (elems 0..15), out[4..7] = high
// nibbles (elems 16..31). out[i] holds block elements 4i..4i+3 — the same
// sequential ordering as the q8_0 dequant, so the V accumulate is unchanged.
...
#else
        {
            // Default: dequant q4_0 V → half in local memory. out[g] (g<4) low
            // nibbles (elems 0..15), out[4+g] high nibbles (elems 16..31), so
            // l_v[row][blk*8 + m] = block elements 4m..4m+3 (matches the q8 path).
...
__kernel void flash_attn_f32_q8q4_merge(
