...
// The pack copy repeated over get_group_id(1) slots: slot s moves the same
// shape from src0 + s*src_step to dst + s*dst_step (byte steps, either sign).
// Serves a run of consecutive cpy nodes that differ only in their offsets, such
// as the K conv-state snapshots a delta-net layer writes into its rollback
// slots, where the source window walks backwards while the slot walks forwards.
kernel void kernel_cpy_f32_f32_pack_slots(
        global float * src0,
        ulong offset0,
        global float * dst,
        ulong offsetd,
