...
// -cl-finite-math-only, under which exp(-inf - -inf) for a fully-masked key
// block (which occurs on SWA models like gpt-oss) is undefined and leaks.
// Mirror the quant-KV FA kernels: large finite negative + clamped softmax max.
...
__kernel void flash_attn_f16(
    ...
    for (int k_start = 0; k_start < n_kv; k_start += BLOCK_N) {
        ...
        for (int j = 0; j < BLOCK_N; j += 2) {
            ...
            // Clamp the exp() reference so a fully-masked key block (m_new ==
            // FA_M_INIT) yields exp(-inf)=0 instead of exp(-inf - -inf)=nan
