...
kernel void kernel_mul_mm_nvfp4_f32_l4_lm(
    global char   * src0,
    ulong offset0,
    global float4 * src1,
    ulong offset1,
    ...
    global block_nvfp4 * src0_b = (global block_nvfp4 *)(src0 + offset0);
    ...
    for (int block = 0; block < ne00; block += BK) {
        for (int l = 0; l < BM; l += loadstride_a) {
            if (ir*BM + loadc_a + l < ne01) {
                ...
                global block_nvfp4 * xb = src0_b + ib;

                // 4 sub blocks of 16, low nibbles then high nibbles
                int sb = e >> 4;
                int rem = e & 15;
                int j  = rem & 7;
                int hi = (rem >> 3) & 1;

                float d = ue4m3_to_fp32(xb->d[sb]);

                uchar4 q = vload4(0, xb->qs + 8*sb + j);
                uchar4 n = hi ? (uchar4)(q.s0 >> 4, q.s1 >> 4, q.s2 >> 4, q.s3 >> 4)
                              : (uchar4)(q.s0 & 0xf, q.s1 & 0xf, q.s2 & 0xf, q.s3 & 0xf);

                float4 v1 = (float4)(kvalues_fp4[n.s0], kvalues_fp4[n.s1],
                                     kvalues_fp4[n.s2], kvalues_fp4[n.s3]) * d;
