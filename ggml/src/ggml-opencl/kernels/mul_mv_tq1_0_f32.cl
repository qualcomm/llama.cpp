...
kernel void kernel_mul_mv_tq1_0_f32(
    ...
    global block_tq1_0 * x = (global block_tq1_0 *) (src0 + offset_src0);
    ...
    for (int ib = ix; ib < nb; ib += BLOCK_STRIDE) {
        ...
        for (int row = 0; row < N_DST && first_row + row < ne01; row++) {
            global block_tq1_0 * xb = (global block_tq1_0 *)(xrow + row*nb01);
            ...
            for (int k = 0; k < 32; ++k) {
                ...
                int   n;
                uchar b;
                if (e < 160)      { n = e >> 5;                       b = xb->qs[e & 31]; }
                else if (e < 240) { int e2 = e - 160; n = e2 >> 4;    b = xb->qs[32 + (e2 & 15)]; }
                else              { int e3 = e - 240; n = e3 >> 2;    b = xb->qh[e3 & 3]; }
                uchar q = (uchar)(b * pow3[n]);
                acc += yl[k] * (float)((int)(((uint)q * 3) >> 8) - 1);
            }
