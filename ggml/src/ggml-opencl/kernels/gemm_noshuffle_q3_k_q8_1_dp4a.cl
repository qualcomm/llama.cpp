...
#define LD4(arr, b) ((float4)((float)arr[(b)+0], (float)arr[(b)+1], (float)arr[(b)+2], (float)arr[(b)+3]))
        ...
        for (int g = 0; g < NGROUPS; ++g) {
            const int b = g * 4;
            float4 rf;
            rf.s0 = dl0 * (float)dot4_q8a(qlo, sh_qa[b+0]) + dl1 * (float)dot4_q8a(qhi, sh_qa[b+0] + 4);
            rf.s1 = dl0 * (float)dot4_q8a(qlo, sh_qa[b+1]) + dl1 * (float)dot4_q8a(qhi, sh_qa[b+1] + 4);
            rf.s2 = dl0 * (float)dot4_q8a(qlo, sh_qa[b+2]) + dl1 * (float)dot4_q8a(qhi, sh_qa[b+2] + 4);
            rf.s3 = dl0 * (float)dot4_q8a(qlo, sh_qa[b+3]) + dl1 * (float)dot4_q8a(qhi, sh_qa[b+3] + 4);
            acc[g] += LD4(sh_d, b) * rf;
        }
#undef LD4
        barrier(CLK_LOCAL_MEM_FENCE);
