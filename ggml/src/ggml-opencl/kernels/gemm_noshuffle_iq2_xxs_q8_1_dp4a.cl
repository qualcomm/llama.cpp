...
// IQ2XXS_GEMM_LDSGRID=1: stage the grid in local memory. OFF by default, and the
// reasoning is in the IQ2_S twin: a 64-thread workgroup pays far more occupancy
// for the LDS than the 512-thread GEMV does, and each lookup here already feeds
// 32 columns. This grid is the smallest of the set at 2 KB, so it has even less
// to gain. Kept only so it can be re-tested cheaply.
// IQ2XXS_GEMM_GRIDIMG=1: read the codebook through an image1d_buffer.
//
...
#ifndef IQ2XXS_GEMM_LDSGRID
#define IQ2XXS_GEMM_LDSGRID 0
#endif
...
kernel void kernel_gemm_noshuffle_iq2_xxs_q8_1_dp4a(
        __read_only image1d_buffer_t grid_img,   // see IQ2XXS_GEMM_GRIDIMG
