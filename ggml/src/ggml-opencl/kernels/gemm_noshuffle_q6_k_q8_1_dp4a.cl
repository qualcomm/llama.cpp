...
// Same kernel with the activation tile staged as uint4. The eight uints a token
// needs for one 32-K step are contiguous, so two uint4 loads replace eight uint
// ones: a quarter as many __local loads in the inner loop and a 16-byte instead
// of a 4-byte cooperative global load. Identical arithmetic, identical results.
