...
// Split-K form of the kernel above for outputs too narrow to fill the GPU: m rows make m/64
// workgroups, and at m = 2560 that is 40 of them on 16 compute units. Workgroup ks along the
// second grid axis takes every ksplit-th group of sub-blocks and writes its partial sum to
// partial[ks*ne01 + row]; kernel_gemv_splitk_reduce_f32 adds the slices.
__attribute__((qcom_reqd_sub_group_size("half")))
__kernel void gemv_noshuffle_q4_k_f32_32b_trans_splitk(
    read_only image1d_buffer_t src0_q,
    __global half *         src0_d,
    __global half *         src0_dm,
    __global uchar *        src0_s,
    ...
    for (uint ib = ks * N_SIMDGROUP + sgid; ib < num_subblocks; ib += N_SIMDGROUP * ksplit) {
        uint sb = ib / 8;
        uint j  = ib % 8;
