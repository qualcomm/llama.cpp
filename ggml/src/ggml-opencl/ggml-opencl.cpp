...
struct ggml_backend_opencl_context {
    ...
    bool adreno_use_bin_kernels = false;
    ...
    // prealloc buffers for MoE router-table preprocess (mxfp4 _ns path)
    ...
    std::map<std::pair<int, int>, cl_kernel> kernels_flash_attn_f32_q8_0;
    ...
    cl_kernel kernel_dequant_q8_0_f16_view_aos, kernel_dequant_q4_0_f16_view_aos;
...
// bin_size>0 loads a precompiled program binary (clCreateProgramWithBinary)
// instead of compiling from source; used for the optional QPM Adreno bin kernels.
...
static void load_cl_kernels(ggml_backend_opencl_context *backend_ctx, ggml_cl_version opencl_c_version) {
    ...
    // flash_attn
    {
        ...
        if (!kernel_src_f16.empty() && !kernel_src_f32.empty() && !kernel_src_f32_f16.empty()) {
            ...
            for (size_t i = 0; i < sizeof(fa_dims)/sizeof(fa_dims[0]); ++i) {
                ...
                // Native q8_0 KV-cache flash-attention (non-split path; N_SPLIT defaults to 1).
                if (!kernel_src_f32_q8_0.empty()) {
                    cl_program prog_f32_q8_0 = build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src_f32_q8_0.c_str(), OPTS);
                    cl_kernel k_f32_q8_0, k_f32_q8_0_q1;
                    CL_CHECK((k_f32_q8_0 = clCreateKernel(prog_f32_q8_0, "flash_attn_f32_q8_0", &err), err));
                    CL_CHECK((k_f32_q8_0_q1 = clCreateKernel(prog_f32_q8_0, "flash_attn_f32_q8_0_q1", &err), err));
                    backend_ctx->kernels_flash_attn_f32_q8_0[{dk, dv}] = k_f32_q8_0;
                    ...
                    // native quant FA prefill kernel to skip fully-masked KV tiles
                    // (essential to stay under the GPU TDR budget at long context).
                    if (!kernel_src_pre_f16.empty()) {
                        cl_program prog_pre = build_program_from_source(backend_ctx->context, backend_ctx->device, kernel_src_pre_f16.c_str(), OPTS);
                        cl_kernel k_blk;
                        CL_CHECK((k_blk = clCreateKernel(prog_pre, "flash_attn_blk_f16", &err), err));
                        backend_ctx->kernels_flash_attn_blk_f16[{dk, dv}] = k_blk;
                        CL_CHECK(clReleaseProgram(prog_pre));
                    }
                }
    ...
    // gemm_moe_mxfp4_f32_ns_bin (QPM precompiled binary kernel, optional)
    {
        size_t bin_size = 0;
    ...
    // gemm_moe_q4_0_f32_ns_bin (QPM precompiled binary kernel, optional)
    {
        size_t bin_size = 0;
    ...
    // gemm_moe_q4_1_f32_ns_bin (QPM precompiled binary kernel, optional)
    {
        size_t bin_size = 0;
...
static ggml_backend_opencl_context * ggml_cl2_init(ggml_backend_dev_t dev) {
    ...
    // if it fails to load, builtin kernels will be used
    {
...
struct ggml_tensor_extra_cl_q4_1 {
    cl_mem q = nullptr;
    cl_mem q_img = nullptr;
    cl_mem d = nullptr;     // scales
    cl_mem d_img = nullptr;
    cl_mem m = nullptr;     // mins
...
#pragma message("TODO: implement BF16, Q4_0, Q4_1, Q5_0, Q5_1, Q8_0, IQ4_NL support (https://github.com/ggml-org/llama.cpp/pull/14661)")
        ...
        case GGML_OP_MUL_MAT_ID:
            ...
            // q4_1 only has the Adreno MoE (_ns) expert path, no generic kernel.
            if (op->src[0]->type == GGML_TYPE_Q4_1) {
                const bool is_moe_expert =
                    ((strstr(op->src[0]->name, "ffn") != NULL) || (strstr(op->src[0]->name, "as") != NULL)) &&
                    (op->src[0]->ne[1] % 64 == 0);
                if (is_moe_expert && op->src[1]->type == GGML_TYPE_F32) {
                    return ggml_is_contiguous(op->src[0]) && ggml_is_contiguous(op->src[1]);
                }
            }
            return false;
        ...
        case GGML_OP_FLASH_ATTN_EXT:
            {
                ...
                // f32-query KV matrix: K and V may each be f16 / q8_0 / q4_0,
                // including asymmetric (k->type != v->type). Native kernels handle
                // the symmetric q8_0/q4_0 pairs; everything else GPU-dequantises the
                // quant side(s) to f16 and runs the f32_f16 kernel.
                auto kv_type_ok = [](ggml_type t) {
                    return t == GGML_TYPE_F16 || t == GGML_TYPE_Q8_0 || t == GGML_TYPE_Q4_0;
                };
                const bool is_f32_quant_kv = q->type == GGML_TYPE_F32 && op->type == GGML_TYPE_F32 &&
                                        kv_type_ok(k->type) && kv_type_ok(v->type);

                return is_f32_f32 || is_f16_f16 || is_f32_f16 || is_f32_quant_kv;
            }
        default:
            return false;
    }
...
inline bool use_adreno_moe_kernels(const ggml_backend_opencl_context *backend_ctx, const ggml_tensor *tensor) {
    ...
    return ((strstr(tensor->name, "ffn") != NULL) || (strstr(tensor->name, "as") != NULL)) && (ne01 % 32 == 0);
}
...
static void ggml_backend_opencl_buffer_set_tensor(ggml_backend_buffer_t buffer, ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (tensor->type == GGML_TYPE_Q4_0) {
...
#ifdef GGML_OPENCL_USE_ADRENO_KERNELS
        // Adreno MoE q4_0 expert weights use the noshuffle (_ns) transposed
        // layout consumed by kernel_gemm/gemv_moe_q4_0_f32_ns.
        if (use_adreno_moe_kernels(backend_ctx, tensor)) {
            ...
            // Create image for Q (transposed _ns layout)
    ...
    if (tensor->type == GGML_TYPE_Q4_1) {
        ...
        // The q4_1 block stores d, then m, then quants. Lay out the SoA planes
        // as scales, mins, quants over the original tensor backing buffer.
        cl_buffer_region region;
...
#ifdef GGML_OPENCL_USE_ADRENO_KERNELS
        // Adreno MoE q4_1 expert weights use the noshuffle (_ns) transposed layout.
        if (use_adreno_moe_kernels(backend_ctx, tensor)) {
        ...
        // non-MoE q4_1: basic SoA convert
...
#ifdef GGML_OPENCL_USE_ADRENO_KERNELS
        // MoE q4_0 expert weights were stored in the noshuffle (_ns) layout;
        // restore them with the matching kernel before the dense path.
        if (use_adreno_moe_kernels(backend_ctx, tensor)) {
            cl_int err;
            cl_mem data_device = clCreateBuffer(context, CL_MEM_READ_WRITE,
                ggml_nbytes(tensor), NULL, &err);
...
static void ggml_cl_set_rows(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    switch (dst->type) {
        ...
        case GGML_TYPE_F16:
            ...
            break;
        case GGML_TYPE_Q8_0:
            // Quantize-on-write into a q8_0 (KV cache) destination. The KV cache is
            // a plain on-device buffer (AoS block_q8_0), so the SoA path is not used;
            // the kernel takes the same args as the f32/f16 variants.
            if (src1->type == GGML_TYPE_I64) {
                kernel = backend_ctx->kernel_set_rows_q8_0_i64;
            } else {
                kernel = backend_ctx->kernel_set_rows_q8_0_i32;
            }
            break;
        case GGML_TYPE_Q4_0:
            // Quantize-on-write into a q4_0 (KV cache) destination (AoS block_q4_0).
            if (src1->type == GGML_TYPE_I64) {
...
// RAII holder for a flash-attention temporary device buffer.
...
// GPU-dequantise a (possibly strided-view) q8_0/q4_0 KV tensor into a tight f16
// temp buffer, honouring the view offset/strides. Used by the asymmetric-KV
// flash-attention fallback so the f32_f16 (mixed) kernel can read a quant side.
// On success repoints buf/offset/nb1..3 at the f16 temp and returns true.
static bool ggml_cl_flash_attn_dequant_kv_to_f16(
        ggml_backend_opencl_context *    backend_ctx,
        const ggml_tensor *              tensor,
    ...
    const cl_int nblk0 = (cl_int) (tensor->ne[0] / 32);
    const cl_int ne1   = (cl_int) tensor->ne[1];
    const cl_int ne2   = (cl_int) tensor->ne[2];
    const cl_int ne3   = (cl_int) tensor->ne[3];
    ...
    cl_kernel kernel = is_q8_0 ? backend_ctx->kernel_dequant_q8_0_f16_view_aos
                               : backend_ctx->kernel_dequant_q4_0_f16_view_aos;
    CL_CHECK(clSetKernelArg(kernel, 0, sizeof(cl_mem),   &buf));
    CL_CHECK(clSetKernelArg(kernel, 1, sizeof(cl_ulong), &offset));
    CL_CHECK(clSetKernelArg(kernel, 2, sizeof(cl_ulong), &nb1));
    CL_CHECK(clSetKernelArg(kernel, 3, sizeof(cl_ulong), &nb2));
    CL_CHECK(clSetKernelArg(kernel, 4, sizeof(cl_ulong), &nb3));
    CL_CHECK(clSetKernelArg(kernel, 5, sizeof(cl_int),   &nblk0));
    CL_CHECK(clSetKernelArg(kernel, 6, sizeof(cl_int),   &ne1));
    CL_CHECK(clSetKernelArg(kernel, 7, sizeof(cl_int),   &ne2));
    CL_CHECK(clSetKernelArg(kernel, 8, sizeof(cl_int),   &ne3));
    CL_CHECK(clSetKernelArg(kernel, 9, sizeof(cl_mem),   &temp.data));

    size_t gws[3] = { (size_t) nblk0, (size_t) ne1, (size_t) ne2 * (size_t) ne3 };
    CL_CHECK(clEnqueueNDRangeKernel(backend_ctx->queue, kernel, 3, NULL, gws, NULL, 0, NULL, NULL));

    // Tight f16 layout produced by the dequant kernel.
    buf    = temp.data;
    offset = 0;
    nb1    = (cl_ulong) tensor->ne[0] * sizeof(ggml_fp16_t);
    nb2    = nb1 * (cl_ulong) tensor->ne[1];
    nb3    = nb2 * (cl_ulong) tensor->ne[2];
    return true;
}
...
static void ggml_cl_flash_attn(ggml_backend_t backend, const ggml_tensor * q, const ggml_tensor * k, ggml_tensor * dst) {
    ...
    const bool v_quant = v->type == GGML_TYPE_Q8_0 || v->type == GGML_TYPE_Q4_0;
    // Native quant KV-cache flash-attention: q is f32, K and V are both the same
    ...
    cl_mem k_buffer = extra_k->data_device;
    cl_mem v_buffer = extra_v->data_device;
    ...
    // Asymmetric / non-native quant KV: GPU-dequantise each quant side to a tight
    // f16 temp buffer; the f16 side (if any) is read as-is. The f32_f16 kernel
    // then reads both K and V as f16. Temp buffers live until the kernel is
    // enqueued (released at function scope).
    ggml_cl_flash_attn_temp_buffer temp_k_f16;
    ggml_cl_flash_attn_temp_buffer temp_v_f16;
    if (use_dequant) {
        if (k_quant) {
            GGML_ASSERT(ggml_cl_flash_attn_dequant_kv_to_f16(
                backend_ctx, k, temp_k_f16, k_buffer, offset_k, k_nb1, k_nb2, k_nb3));
        }
        if (v_quant) {
            GGML_ASSERT(ggml_cl_flash_attn_dequant_kv_to_f16(
                backend_ctx, v, temp_v_f16, v_buffer, offset_v, v_nb1, v_nb2, v_nb3));
        }
    }
    ...
    CL_CHECK(clSetKernelArg(kernel, 2, sizeof(cl_mem),   &k_buffer));
    CL_CHECK(clSetKernelArg(kernel, 3, sizeof(cl_ulong), &offset_k));
    CL_CHECK(clSetKernelArg(kernel, 4, sizeof(cl_mem),   &v_buffer));
    ...
    // The prefill (n_q>1) q8_0 kernel takes one extra arg: the per-(q-tile,kv-tile)
    // mask-class buffer from the prepass. When a mask is present we run the blk
    ...
    if ((is_q8_0 || is_q4_0 || is_q8q4) && n_q > 1) {
        ...
        if (mask_buffer != NULL && blk_it != backend_ctx->kernels_flash_attn_blk_f16.end()) {
            ...
            cl_int blk_err;
            blk_buffer = clCreateBuffer(backend_ctx->context, CL_MEM_READ_WRITE, blk_size, NULL, &blk_err);
            if (blk_err != CL_SUCCESS) {
                CL_CHECK(clFinish(backend_ctx->queue));
                blk_buffer = clCreateBuffer(backend_ctx->context, CL_MEM_READ_WRITE, blk_size, NULL, &blk_err);
            }
            CL_CHECK(blk_err);

            cl_kernel kernel_blk = blk_it->second;
            ...
            size_t blk_global[] = { (size_t)n_kv_blocks, (size_t)n_q_blocks, (size_t)(mask_ne2 * mask_ne3) };
            backend_ctx->enqueue_ndrange_kernel(kernel_blk, 3, blk_global, NULL, dst);
        }
    ...
    // The enqueued FA kernel retains the blk buffer until it completes; release our ref.
    if (blk_buffer != NULL) {
        CL_CHECK(clReleaseMemObject(blk_buffer));
    }
}
...
static void ggml_cl_mul_mat_id(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    // MoE router-table tiling for the Adreno mxfp4 _ns GEMM path.
    const int n_tile_size = 32;
    const int max_post_router_tile = (ne20 * ne21 / n_tile_size) + ne02;
...
#endif // GGML_OPENCL_USE_ADRENO_KERNELS
            GGML_ASSERT(false && "q4_1 mul_mat_id requires the Adreno MoE (_ns) path");
        }
        case GGML_TYPE_Q8_0: {
#ifdef GGML_OPENCL_SOA_Q
...
#ifdef GGML_OPENCL_USE_ADRENO_KERNELS
    // A fresh MoE router was produced; mark the post-router tile cache stale so
    // the next mxfp4 _ns mul_mat_id re-runs moe_router_reoerder.
