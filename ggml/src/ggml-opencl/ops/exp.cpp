...
void ggml_cl_load_kernels_exp(ggml_backend_opencl_context * backend_ctx) {
    ...
    // exp
    {
        ...
        CL_CHECK((backend_ctx->exp.kernel_exp_f32    = clCreateKernel(prog, "kernel_exp_f32", &err), err));
        CL_CHECK((backend_ctx->exp.kernel_exp_f32_4  = clCreateKernel(prog, "kernel_exp_f32_4", &err), err));
        CL_CHECK((backend_ctx->exp.kernel_exp_f32_nc = clCreateKernel(prog, "kernel_exp_f32_nc", &err), err));
        CL_CHECK((backend_ctx->exp.kernel_exp_f16    = clCreateKernel(prog, "kernel_exp_f16", &err), err));
        CL_CHECK((backend_ctx->exp.kernel_exp_f16_4  = clCreateKernel(prog, "kernel_exp_f16_4", &err), err));
        CL_CHECK((backend_ctx->exp.kernel_exp_f16_nc = clCreateKernel(prog, "kernel_exp_f16_nc", &err), err));
    ...
    if (ggml_is_contiguous(src0)) {
        ...
        if (n % 4 == 0) {
            if (src0->type == GGML_TYPE_F32) {
                kernel = backend_ctx->exp.kernel_exp_f32_4;
            } else {
                kernel = backend_ctx->exp.kernel_exp_f16_4;
            }
            n /= 4;
        } else {
            if (src0->type == GGML_TYPE_F32) {
                kernel = backend_ctx->exp.kernel_exp_f32;
            } else {
                kernel = backend_ctx->exp.kernel_exp_f16;
            }
        }
        ...
        if (src0->type == GGML_TYPE_F32) {
            kernel = backend_ctx->exp.kernel_exp_f32_nc;
        } else {
            kernel = backend_ctx->exp.kernel_exp_f16_nc;
        }
