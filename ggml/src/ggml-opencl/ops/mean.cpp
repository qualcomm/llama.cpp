...
void ggml_cl_load_kernels_mean(ggml_backend_opencl_context * backend_ctx) {
    ...
    // mean
    {
        ...
        CL_CHECK((backend_ctx->mean.kernel_mean_f32 = clCreateKernel(prog, "kernel_mean_f32", &err), err));
        CL_CHECK((backend_ctx->mean.kernel_mean_f32_4 = clCreateKernel(prog, "kernel_mean_f32_4", &err), err));
...
void ggml_cl_mean(ggml_backend_t backend, const ggml_tensor * src0, const ggml_tensor * src1, ggml_tensor * dst) {
    ...
    if (is_c4) {
        kernel = backend_ctx->mean.kernel_mean_f32_4;
    } else {
        kernel = backend_ctx->mean.kernel_mean_f32;
    }
