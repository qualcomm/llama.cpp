    ...
    void draft(common_speculative_draft_params_vec & dparams) override {
        ...
        for (llama_seq_id seq_id = 0; seq_id < (llama_seq_id) n_seq; ++seq_id) {
            ...
            for (int32_t i = 0; i < n_block_tokens; ++i) {
                // DFlash2 does not want logits on the masked block rows; for DFlash1 the
                // env-gated row cap applies as before (logits_row is true unless capped).
                common_batch_add(batch, i == 0 ? dp.id_last : mask_token_id, n + i, { seq_id }, !is_dflash2 && logits_row(i));
            }
        }
