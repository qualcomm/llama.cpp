...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // lm_head-shape decode GEMV (m=vocab, n=1, k=n_embd): exercises the tiled
    // q4_K/q6_K GEMV (NSUBGROUPS=8 K-split) + the q8_0 path. Correctness vs CPU.
    ...
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_F16, GGML_TYPE_F32, 1056, 1, 129, {1,  1}, {1, 1}));
    test_cases.emplace_back(new test_mul_mat(GGML_TYPE_F16, GGML_TYPE_F32, 1056, 1, 193, {1,  1}, {1, 1}));
    ...
    // GQA=8 DK=DV=128 f16-KV decode at n_kv>=2048 — exercises the Intel g8 c8
    // cluster-parallel decode FA (Qwen2.5/Qwen3 GQA=8 class); validates the
    ...
    // head size 128 past one chunk of 64 tokens (chunked prefill kernels): whole chunks, tails, snapshots
    ...
    // coverage at any head size, so a backend that special-cases decode
    // attention per (head size, ratio) could silently have no path for it.
    for (int kv : { 4096, 4099 }) {
...
static std::vector<std::unique_ptr<test_case>> make_test_cases_perf() {
 ...
 // High resolution output (large NPQ)
    ...
    // Mamba-2 / gated-delta-net recurrent-state cache copy: ONE row of ne00 elements. The
    // OpenCL cpy maps one WORKGROUP per row, so a single-row copy of this size runs on one
    // compute unit no matter how wide the workgroup is -- every f32->f32 case above has 256+
    // rows and cannot show it. Nemotron-H issues 23 of these per graph (one per SSM layer,
    // 2 MB each) and they account for ~14% of its GPU time. The 3-slot form is what a
    // speculative verify produces.
    ...
    // Qwen3.8-27B and Nemotron-3.5-Lightning-30B-A3B, the two Q4_0 deployment targets.
    // Neither runs the shapes above, and the small-batch K-split optimum depends on how
    // many workgroups the shape launches, so it has to be measured where they live.
    for (int bs : {1, 2, 3, 4, 5, 8, 512}) {
        for (ggml_type type_a : {GGML_TYPE_Q4_0, GGML_TYPE_Q4_K}) {
            test_cases.emplace_back(new test_mul_mat(type_a, GGML_TYPE_F32, 17408, bs,  5120, {1, 1}, {1, 1}));
            test_cases.emplace_back(new test_mul_mat(type_a, GGML_TYPE_F32,  5120, bs, 17408, {1, 1}, {1, 1}));
            test_cases.emplace_back(new test_mul_mat(type_a, GGML_TYPE_F32,  6144, bs,  5120, {1, 1}, {1, 1}));
    ...
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 16, 128, 512, 1, 2));  // Qwen3.5-35B: 16 k-heads x 2, PP-512
    test_cases.emplace_back(new test_gated_delta_net(GGML_TYPE_F32, 16, 128, 1024, 1, 2)); // Qwen3.5-35B PP-1024
