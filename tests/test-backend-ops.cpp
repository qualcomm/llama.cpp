...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
        ...
        for (int hs : { 64, 128, 256, }) {
            for (int nb : { 32, 64, }) {
                for (ggml_type type_KV : { GGML_TYPE_F16, GGML_TYPE_Q8_0, GGML_TYPE_Q4_0, }) {
                    test_cases.emplace_back(new test_flash_attn_ext(hs, hs, 8, {4, 1}, kv, nb, true, false, 0, 0, GGML_PREC_F32, type_KV));
                }
            }
        }
    }
    ...
    for (int kv : { 2048, 4096, }) {
        for (int nb : { 2, 4, 8, }) {
            test_cases.emplace_back(new test_flash_attn_ext(128, 128, 4, {8, 1}, kv, nb, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16));
        }
    }
    for (int nb : { 2, 8, }) {
        test_cases.emplace_back(new test_flash_attn_ext(64, 64, 8, {4, 1}, 2048, nb, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16));
    }
    ...
    for (int kv : { 4096, 4099 }) {
        test_cases.emplace_back(new test_flash_attn_ext(512, 512, 1, {16, 1}, kv, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16));
    }
    ...
    for (int kv : { 4096, 4099 }) {
        test_cases.emplace_back(new test_flash_attn_ext(256, 256, 8, {2, 1}, kv, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16));
        test_cases.emplace_back(new test_flash_attn_ext(256, 256, 8, {2, 1}, kv, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_Q8_0));
        test_cases.emplace_back(new test_flash_attn_ext(512, 512, 2, {8, 1}, kv, 1, true, false, 0, 0, GGML_PREC_F32, GGML_TYPE_F16));
    }
